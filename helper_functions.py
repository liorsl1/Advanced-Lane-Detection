...
def binary_thresholding(img, s_thresh=(170, 250), sx_thresh=(30, 150)):
...
def draw_lines(undist, warped, yvals, left_fitx, right_fitx, right_lane_inds, left_lane_inds, Minv):
...
def annotate_result(result):
    global line_prev
    top_lane_center = (line_prev.left_x_top + line_prev.right_x_top) / 2
    xcenter = result.shape[1]/2
    offset = xcenter - top_lane_center
    vehicle_offset =  offset * xm_per_pix

    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(result, 'Mean Radius of curvature (Left)  = %.2f m' % (line_prev.mean_left_curverad),
        (10, 40), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(result, 'Mean Radius of curvature (Right) = %.2f m' % (line_prev.mean_left_curverad),
...
def process_image(img):
    ...
    s_thresh = (170, 250)
    sx_thresh = (30, 150)
    img_warped, M = perspective_transform(undist_img)
    ...
    if line_prev is not None :
        ...
        if line is None or is_good_fit(line_prev, line) is False:
            left_lane_inds, right_lane_inds, out_img = naive_find_lines(binary_warped)
            line = process_fit(binary_warped, left_lane_inds, right_lane_inds)
            num_full_search = num_full_search + 1
    else:
        # find new line with naive full process
        left_lane_inds, right_lane_inds, out_img = naive_find_lines(binary_warped)
        line = process_fit(binary_warped, left_lane_inds, right_lane_inds)
        num_full_search = num_full_search + 1
        
    result = draw_lines(undist_img, binary_warped, line.yvals, line.left_fitx, line.right_fitx, line.right_lane_inds, line.left_lane_inds, np.linalg.inv(M))
    line_prev = line
    annotated_result = annotate_result(result)
...
def process_video():
    global num_full_search, mtx, dist, line
    mtx, dist = calibrate_camera("./camera_cal/calibration*.jpg")
    assert (mtx is not None), (dist is not None)
    output_name = 'cha_video_output.mp4'
